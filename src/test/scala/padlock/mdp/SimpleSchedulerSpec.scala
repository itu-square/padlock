...
// Manual testing was suspicious so far.
  ...
  "Probabilistic Boolean choice"  - {
    ...
    "is referentially transparent (2)" in check {
      Prop.forAll  { seed: Long =>
        val s = SimpleScheduler.empty[Unit, Unit] (seed)
  ...
  "Demonic Boolean choice (FairCoinScheduler)" - {

    "can resolve to false" in check {
      Prop.exists  { seed: Long =>
        !SimpleScheduler
          .FairCoinScheduler[Unit, Unit] (seed)
          .demonic (())
          ._2
          .getOrElse (true)
      }
